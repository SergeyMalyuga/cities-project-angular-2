...
export class OfferComponent implements OnInit {
  ...
  public isFavorite = toSignal(
    toObservable(this.offerId).pipe(
      switchMap((id) =>
        id ? this.store.select(selectIsFavoriteOffer(id)) : of(false),
      ),
    ),
    { initialValue: false },
  );
  public isFavoriteOffersLoading = this.store.selectSignal(
    selectIsFavoriteOffersIsLoading,
  );
  ...
  public ngOnInit(): void {
    this.activatedRoute.paramMap
      .pipe(
        ...
        switchMap((id) => {
          const offers$ = this.offerService.getOfferById(id).pipe(
            catchError(() => {
          ...
          const comments$ = merge(
            ...
            this.refreshReviews$
              .pipe(switchMap(() => this.reviewsService.getComments(id)))
              .pipe(
                distinctUntilChanged(
                  (prev, curr) =>
          ...
          const neighborOffers$ = this.offerService
            .getNearbyOffers(id)
            .pipe(catchError(() => of([])));
  ...
  public postComment(comment: NewComment) {
    ...
    if (id) {
      this.reviewsService
        .postComment(comment.comment, Number(comment.rating), id)
        .subscribe(() => this.refreshReviews$.next());
    }
  }
