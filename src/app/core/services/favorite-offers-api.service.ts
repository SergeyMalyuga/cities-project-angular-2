...
export class FavoriteOffersApiService {
  ...
  public getOffers(): Observable<OfferPreview[]> {
    return this.http.get<OfferPreview[]>(`${BASE_URL}/${APIRoute.FAVORITE}`).pipe(...defaultHttpPipe<OfferPreview[]>());
  }
  ...
  public toggleFavorite(offerId: string, isFavorite: boolean): Observable<Offer> {
    ...
    return this.http.post<Offer>(`${BASE_URL}/${APIRoute.FAVORITE}/${offerId}/${statusValue}`, {}).pipe(...defaultHttpPipe<Offer>());
  }
}
